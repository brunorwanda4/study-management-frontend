import { AttendanceSystemEnum, ContactSchema, SchoolAddress, SchoolMembers, SchoolTypeEnum, SocialMediaSchema } from "@/lib/schema/school.dto";
import z from "zod";

export const SchoolIdentitySchema = z.object({
  name: z.string().min(1, { message: "School name cannot be empty if provided" }).optional(),
  username: z.string().min(1, { message: "Username cannot be empty if provided" }).optional(),
...
export type SchoolClassificationDto = z.infer<typeof SchoolClassificationSchema>;
...
export type FacilitiesActivitiesDto = z.infer<typeof FacilitiesActivitiesSchema>;


// --- Recomposing the Full Schema ---

// Merge the sub-schemas sequentially to reconstruct the full schema
export const RecomposedPublicSchoolUpdateSchema = SchoolIdentitySchema
  .merge(SchoolClassificationSchema)
...
export type RecomposedPublicSchoolUpdateDto = z.infer<typeof RecomposedPublicSchoolUpdateSchema>;
