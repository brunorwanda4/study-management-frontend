...
export const createSchoolService = async (input: CreateSchoolDto) => {
  const data: CreateSchoolDtoBackend = {
    ...
    sportsExtracurricular: input.sportsExtracurricular?.map(
      (sport) => sport.value,
    ),
    onlineClasses: input.onlineClasses,
  };

  return await apiRequest<CreateSchoolDtoBackend, SchoolDto>(
    "post",
    "/school",
    data,
  );
...
export const academicSchoolService = async (input: schoolAcademicDto) => {
  const data: SchoolAcademicDtoBackend = {
    ...
    aLevelSubjectCombination: input.aLevelSubjectCombination?.map(
      (item) => item.value,
    ),
  ...
  return await apiRequest<SchoolAcademicDtoBackend, SchoolAcademicCreationDto>(
    "post",
    "/school/academic",
    data,
  );
...
export const administrationSchoolRequestToJoinSchool = async (
  input: SchoolAdministrationDto,
) => {
  return await apiRequest<
    SchoolAdministrationDto,
    sendAdministrationJoinRequestsDto
  >("post", `/school/administration`, input);
};

export const updateSchoolSchoolService = async (
  id: string,
  input: PublicSchoolUpdateDto,
) => {
  return await apiRequest<PublicSchoolUpdateDto, SchoolDto>(
    "patch",
    `/school/${id}`,
    input,
  );
